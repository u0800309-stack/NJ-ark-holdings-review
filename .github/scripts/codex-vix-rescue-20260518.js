...
(async () => {
  ...
  const fn = `async function fetchVixOnce(){var box=document.querySelector("#vixTicker"),val=document.querySelector("#vixValue"),time=document.querySelector("#vixTime");if(!box||!val||!time)return null;var fallbackValue=${fallbackNumber},fallbackSource=${JSON.stringify(fallbackSource)};function apply(v,src){v=Number(v);if(!Number.isFinite(v)||v<=0)throw new Error("bad vix");state.lastVix={value:v,time:new Date(),source:src};val.textContent=v.toFixed(2);time.textContent=src;box.classList.toggle("hot",v>=25);box.classList.toggle("calm",v<18);return state.lastVix}function timeout(ms){return new Promise(function(_,rej){setTimeout(function(){rej(new Error("timeout"))},ms)})}async function yahooProxy(){var u="https://api.allorigins.win/raw?url="+encodeURIComponent("https://query1.finance.yahoo.com/v8/finance/chart/%5EVIX?range=1d&interval=1m&_="+Date.now());var r=await Promise.race([fetch(u,{cache:"no-store"}),timeout(3600)]);if(!r.ok)throw new Error("status");var d=await r.json();var x=d.chart&&d.chart.result&&d.chart.result[0];var q=(x&&x.indicators&&x.indicators.quote&&x.indicators.quote[0]&&x.indicators.quote[0].close)||[];var last=[].concat(q).reverse().find(function(n){return Number.isFinite(Number(n))});return apply((x&&x.meta&&x.meta.regularMarketPrice)||last,"Yahoo 最新")}async function cboeProxy(){var u="https://api.allorigins.win/raw?url="+encodeURIComponent("https://cdn.cboe.com/api/global/delayed_quotes/charts/historical/_VIX.json?_="+Date.now());var r=await Promise.race([fetch(u,{cache:"no-store"}),timeout(4200)]);if(!r.ok)throw new Error("status");var d=await r.json();var rows=Array.isArray(d.data)?d.data:[];var last=rows.slice().reverse().find(function(x){return Number.isFinite(Number(x.close))});return apply(last&&last.close,"Cboe 最新")}var immediate=apply(fallbackValue,fallbackSource);Promise.any([yahooProxy(),cboeProxy()]).catch(function(){});return immediate}`;
  ...
  console.log('VIX rescue applied instantly. fallback=' + fallbackNumber + ' source=' + fallbackSource);
})();
